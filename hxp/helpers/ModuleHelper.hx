...
class ModuleHelper {
		...
		for (module in project.modules) {
			...
			if (module.classNames.length > 0) {
				...
				if (excludeTypes.length > 0) {
					...
					hxml += "\n--macro hxp.helpers.ModuleHelper.exclude(['" + excludeTypes.join ("','") + "'])";
					
				}
				
				// order by short filters first, so they match earlier
				haxe.ds.ArraySort.sort(includeTypes, shortFirst);
				hxml += "\n--macro hxp.helpers.ModuleHelper.expose(['" + includeTypes.join ("','") + "'])";
				//hxml += "\n--macro hxp.helpers.ModuleHelper.generate()";
	...
	public static function updateProject (project:HXProject):Void {
		...
		if (excludeTypes.length > 0) {
			...
			// order by short filters first, so they match earlier
			haxe.ds.ArraySort.sort(excludeTypes, shortFirst);
			project.haxeflags.push ("--macro hxp.helpers.ModuleHelper.exclude(['" + excludeTypes.join ("','") + "'])");
			
		}
		
		//if (hasModules) {
			//
			//project.haxeflags.push ("--macro hxp.helpers.ModuleHelper.generate()");
			//
		//}
