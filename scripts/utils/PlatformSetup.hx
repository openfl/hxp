...
class PlatformSetup {
	...
	public static function setupHXP ():Void {
		
		if (!targetFlags.exists ("alias") && !targetFlags.exists ("cli")) {
			
			setupHaxelib (new Haxelib ("hxp"));
		...
		if (PlatformHelper.hostPlatform == Platform.WINDOWS) {
			...
			if (targetFlags.exists ("y")) {
				...
				Sys.println ("Do you want to install the \"hxp\" command? [y/n/a] y");
				
			} else {
				
				answer = CLIHelper.ask ("Do you want to install the \"hxp\" command?");
