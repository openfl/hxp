...
class CommandLineTools {
	...
	public static var commandName = "hxp";
	public static var defaultLibrary = "hxp";
	public static var defaultLibraryName = "HXP";
	...
	private function buildProject (project:Project, command:String = "") {
		...
		if (project.targetHandlers.exists (Std.string (project.target).toLowerCase ())) {
			...
			if (StringTools.endsWith (handler, ".hx")) {
				...
				var buildArgs = [ "-cp", targetDir, className, "-main", "hxp.PlatformTargetMain", "-D", "hxp="+ version, "-cp", PathHelper.combine (HaxelibHelper.getPath (new Haxelib ("hxp")), "src") ];
			...
			if (catchHaxeFlag) {
				...
				if (argument == "-dce" || argument.substr (1, 1) == "-") {
					...
					if (argument.substr (0, 4) == "-arm") {
						...
						overrides.architectures.push (X64);
						
					} else if (argument == "-32") {
						
						overrides.architectures.push (X86);
