...
class Project extends Script {
	...
	public static function fromFile (projectFile:String, userDefines:Map<String, Dynamic> = null, includePaths:Array<String> = null):Project {
		...
		#if lime
		var args = [ name, "-main", "hxp.Project", "-cp", tempDirectory, "-neko", nekoOutput, "-cp", PathHelper.combine (PathHelper.getHaxelib (new Haxelib ("hxp")), "src"), "-lib", "hxp" ];
		#else
		var args = [ name, "--interp", "-main", "hxp.Project", "-cp", tempDirectory, "-cp", PathHelper.combine (PathHelper.getHaxelib (new Haxelib ("hxp")), "src") ];
		#end
