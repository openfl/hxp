package hxp; #if !macro


import haxe.io.Path;
import hxp.Dependency;
import hxp.Haxelib;
import hxp.Project;
import hxp.ModuleData;
import sys.io.File;
...
class ModuleHelper {
		...
		for (module in project.modules) {
			...
			if (module.classNames.length > 0) {
				...
				if (excludeTypes.length > 0) {
					...
					hxml += "\n--macro hxp.ModuleHelper.exclude(['" + excludeTypes.join ("','") + "'])";
					
				}
				
				// order by short filters first, so they match earlier
				haxe.ds.ArraySort.sort(includeTypes, shortFirst);
				hxml += "\n--macro hxp.ModuleHelper.expose(['" + includeTypes.join ("','") + "'])";
				//hxml += "\n--macro hxp.ModuleHelper.generate()";
	...
	public static function updateProject (project:Project):Void {
		...
		if (excludeTypes.length > 0) {
			...
			// order by short filters first, so they match earlier
			haxe.ds.ArraySort.sort(excludeTypes, shortFirst);
			project.haxeflags.push ("--macro hxp.ModuleHelper.exclude(['" + excludeTypes.join ("','") + "'])");
			
		}
		
		//if (hasModules) {
			//
			//project.haxeflags.push ("--macro hxp.ModuleHelper.generate()");
			//
		//}
