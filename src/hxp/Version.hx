...
abstract Version(SemVer) from SemVer to SemVer
{
	...
	@:from public static function stringToVersion(s:String)
	{
		...
		var major = Std.parseInt(VERSION.matched(1)), minor = Std.parseInt(VERSION.matched(2)), patch = Std.parseInt(VERSION.matched(3)),
		pre = parseIdentifiers(VERSION.matched(4)), build = parseIdentifiers(VERSION.matched(5));
	...
	static function equalsIdentifiers(a:Array<Identifier>, b:Array<Identifier>)
	{
		...
		for (i in 0...a.length)
			switch [a[i], b[i]]
			{
				case [StringId(a), StringId(b)] if (a != b):
					return false;
				case [IntId(a), IntId(b)] if (a != b):
					return false;
				case _:
			}
		return true;
...
abstract VersionRule(VersionComparator) from VersionComparator to VersionComparator
{
		...
		var ors = s.split("||").map(function(comp)
		{
			...
			return if (p.length == 1)
			{
				...
				if (p.length == 1)
				{
					...
					{
						// one term pattern
						var v = versionArray(VERSION), vf = v.concat([0, 0, 0]).slice(0, 3);
						switch [VERSION.matched(1), v.length]
						{
							...
							case ["^", 3]:
								...
								AndRule(GreaterThanOrEqualVersion(version),
									LessThanVersion(version.major == 0 ? (version.minor == 0 ? version.nextPatch() : version
										.nextMinor()) : version.nextMajor()));
				...
				else if (p.length == 2)
				{
					...
					var lp = VERSION.matched(1), lva = versionArray(VERSION), lvf = lva.concat([0, 0, 0]).slice(0, 3), lv = Version.arrayToVersion(lvf)
					...
					var rp = VERSION.matched(1), rva = versionArray(VERSION), rvf = rva.concat([0, 0, 0]).slice(0, 3), rv = Version.arrayToVersion(rvf)
						.withPre(VERSION.matched(5), VERSION.matched(6));
	...
	public static function versionRuleIsValid(rule:String)
		return try stringToVersionRule(rule) != null
